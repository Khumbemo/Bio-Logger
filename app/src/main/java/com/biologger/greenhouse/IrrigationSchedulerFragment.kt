...
class IrrigationSchedulerFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        spinnerStage.setAdapter(
            ArrayAdapter(
                requireContext(),
                android.R.layout.simple_dropdown_item_1line,
                arrayOf("Establishment", "Vegetative", "Reproductive", "Maturity")
            )
        )
        ...
        btnSave.setOnClickListener {
            ...
            view.findViewById<TextView>(R.id.resIrrigation).text =
                getString(R.string.label_etc_format).format(etc)
            view.findViewById<TextView>(R.id.resInterval).text =
                getString(R.string.label_recommended_interval)

            cardRes.visibility = View.VISIBLE

            viewModel.insertIrrigationPlan(
                IrrigationPlan(
                    crop = "",
                    stage = spinnerStage.text.toString(),
                    kc = kc,
                    et0 = et0,
                    rainfall = 0.0,
                    soilType = "Loam",
                    ...
                    netRequirement = etc,
                    interval = 3,
                    volumePerEvent = etc * 3,
                    scheduleJson = ""
                )
            )
            Toast.makeText(context, getString(R.string.toast_irrigation_saved), Toast.LENGTH_SHORT).show()
        }
